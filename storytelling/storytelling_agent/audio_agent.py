...
Mirrors the image_agent pattern: an LlmAgent backed by a single tool.
...
audio_agent = LlmAgent(
    ...
    description="Generates audio narration for story chapters using ElevenLabs TTS",
...
- If status is "success": tell the user the audio is ready and share the audio URL(s).
